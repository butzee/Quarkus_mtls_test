...
declare -A services=(
    ["kafka"]="kafka-broker.default.svc.cluster.local,DNS:localhost,DNS:kafka-broker"
    ["microservice-a"]="microservice-a.default.svc.cluster.local, DNS:localhost, DNS: microservice-a"
    ["microservice-b"]="microservice-b.default.svc.cluster.local, DNS:localhost, DNS: microservice-b"
    ["microservice-c"]="microservice-c.default.svc.cluster.local, DNS:localhost, DNS: microservice-c"
    ["microservice-d"]="microservice-d.default.svc.cluster.local, DNS:localhost, DNS: microservice-d"
)
...
create_keystore_and_truststore() {
    ...
    # Generate PEM and .key for microservice-c and microservice-d if applicable
    if [ "$service" == "microservice-c" ] || [ "$service" == "microservice-d" ]; then
