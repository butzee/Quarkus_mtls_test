...
BROKER_CN="kafka.default.svc.cluster.local"

MICROSERVICE_A_CN="microservice-a.default.svc.cluster.local"
MICROSERVICE_B_CN="microservice-b.default.svc.cluster.local"
...
echo "Creating truststore for Microservice-A and Microservice-B..."
keytool -importcert -alias CA -file $CA_CERT -keystore $OUTPUT_DIR/truststore.jks -storepass $PASSWORD -noprompt
echo "Creating truststore for Kafka..."
keytool -importcert -alias CA -file $CA_CERT -keystore $OUTPUT_DIR/kafka.truststore.jks -storepass $PASSWORD -noprompt


echo "Truststores creation completed."
